...
class Datetime(Field):
    ...
    def convert_to_cache(self, value, record, validate=True):
        if not value:
            return False
        if isinstance(value, pycompat.string_types):
            tzinfo = record._context.get('default_tz')
            return self.from_string(value, tzinfo=tzinfo)
    ...
    def convert_to_export(self, value, record):
        if not value:
            return ''
        tzinfo = record._context.get('default_tz')
        return self.from_string(value, tzinfo=tzinfo) if record._context.get('export_raw_data') else ustr(value)

    def convert_to_display_name(self, value, record):
        assert record, 'Record expected'
        tzinfo = record._context.get('default_tz')
        return datetime.to_string(datetime.context_timestamp(record, datetime.from_string(value, tzinfo=tzinfo)))
