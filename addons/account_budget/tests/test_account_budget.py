...
class TestAccountBudget(TestAccountBudgetCommon):
    ...
    def test_account_budget(self):
        ...
        budget = self.env['crossovered.budget'].create({
            'date_from': datetime.datetime.now().get_start_year() + datetime.relativedelta(years=1),
            'date_to': datetime.datetime.now().get_end_year() + datetime.relativedelta(years=1),
