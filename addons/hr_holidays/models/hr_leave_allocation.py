...
class HolidaysAllocation(models.Model):
    ...
    def activity_update(self):
        self.filtered(lambda hol: hol.state == 'draft').activity_unlink(['hr_holidays.mail_act_leave_allocation_approval', 'hr_holidays.mail_act_leave_allocation_second_approval'])
        ...
        for holiday in self.filtered(lambda hol: hol.state == 'validate1'):
            holiday.activity_feedback(['hr_holidays.mail_act_leave_allocation_approval'])
            holiday.activity_schedule(
                'hr_holidays.mail_act_leave_allocation_second_approval', fields.Date.today(),
                user_id=holiday._get_responsible_for_approval().id)
        self.filtered(lambda hol: hol.state == 'validate').activity_feedback(['hr_holidays.mail_act_leave_allocation_approval', 'hr_holidays.mail_act_leave_allocation_second_approval'])
        self.filtered(lambda hol: hol.state == 'refuse').activity_unlink(['hr_holidays.mail_act_leave_allocation_approval', 'hr_holidays.mail_act_leave_allocation_second_approval'])
