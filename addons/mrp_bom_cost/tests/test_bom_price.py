...
class TestBom(common.TransactionCase):
    ...
    def test_01_compute_price_operation_cost(self):
        """Total price of the product should be BoMs cost + Routing(s) cost"""
        ...
        self.workcenter_1 = workcenter_from1.save()

        routing_form1 = Form(self.Routing)
        routing_form1.name = 'Assembly Furniture'
        self.routing_1 = routing_form1.save()
        ...
        # Fitting        (30 / 60) * 100 =  50.00
        # ----------------------------------------
        # Operation Cost  1 dozen (125/2 capacity = 62.5 per dozen) and 5.21 for 1 Unit
        # --------------------------------------------------------------------------
        ...
        self.assertEqual(self.dining_table.standard_price, 1000, "Initial price of the Product should be 1000")
        self.dining_table.compute_price()
        # Total cost of Dining Table = (550) + Total cost of operations (62.5) = 612.5
        self.assertAlmostEquals(self.dining_table.standard_price, 612.5, "After computing price from BoM price should be 612.5")
        self.Product.browse([self.dining_table.id, self.table_head.id]).compute_price()
        # Total cost of Dining Table = (718.75) + Total cost of all operations (62.5 + 5.21) = 786.46
        self.assertAlmostEquals(self.dining_table.standard_price, 786.46, "After computing price from BoM price should be 786.46")

    def test_01_compute_price_inventory_valuation(self):
        """Test for UserError, when action called from tree view
        and Inventory Valuation = Automated"""
        self.glass.categ_id.property_valuation = 'real_time'
        with self.assertRaises(UserError):
            self.dining_table.with_context(button=False).compute_price()
