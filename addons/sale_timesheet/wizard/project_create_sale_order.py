...
class ProjectCreateSalesOrder(models.TransientModel):
    ...
    product_id = fields.Many2one('product.product', domain=[('type', '=', 'service'), ('invoice_policy', '=', 'delivery')])
    partner_id = fields.Many2one('res.partner', domain=[('customer', '=', True)])
    price_unit = fields.Float("Unit price", help="Price unit of the selected product for the generated Sales Order.")
