...
QUnit.test('activity menu widget: activity menu with 4 records', function (assert) {
    ...
    assert.strictEqual(activityMenu.$('.o_reminder_show').hasClass("hidden"), true, 'ActivityMenu add reminder button should be hidden');
    assert.strictEqual(activityMenu.$('.o_reminder').hasClass("hidden"), false, 'ActivityMenu add reminder input should be displayed');
