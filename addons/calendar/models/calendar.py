...
from odoo.tools.datetime import date, datetime, relativedelta, timedelta,\
    rrulestr, parse, DEFAULT_SERVER_DATE_FORMAT, utc_timezone
...
class Meeting(models.Model):
    ...
    def _get_recurrent_date_by_event(self, date_field='start'):
        ...
        return [d.astimezone(utc_timezone) if d.tzinfo else d for d in rset1]
    ...
    def _inverse_dates(self):
        for meeting in self:
            if meeting.allday:
                ...
                enddate = enddate.astimezone(utc_timezone)
                ...
                startdate = startdate.astimezone(utc_timezone)  # Convert to UTC
        ...
        def ics_datetime(idate, allday=False):
            if idate:
                if allday:
                    ...
                    return datetime.from_datetime(idate).replace(tzinfo=utc_timezone).to_pydatetime()
                else:
                    return datetime.from_date(idate).replace(tzinfo=utc_timezone).to_pydatetime()
            return False
    ...
    def get_interval(self, interval, tz=None):
        ...
        date = date.replace(tzinfo=utc_timezone).astimezone(tz)
