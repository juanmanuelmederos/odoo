...
var PagePropertiesDialog = widget.Dialog.extend({
    ...
    save: function(data){
        ...
        $date_publish.parents(".form-group").removeClass('has-error');
        var date_publish = $date_publish.val();
        if (date_publish != "") {
            date_publish = this._parse_date(date_publish);
            if (!date_publish) {
                $date_publish.parents(".form-group").addClass('has-error');
                return;
            }
        }
        var params = {
     ...
     * @returns {String} A string representing an UTC datetime.
     */
